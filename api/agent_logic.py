...
# ============================================================================
# MAIN AGGREGATOR NODE (Using Powerful LLM - Gemini 2.5 Pro)
# ============================================================================

def aggregate_analysis(state: GraphState) -> GraphState:
    """
    Aggregates all specialized analyses into a final comprehensive prediction.
    
    This is the main decision-maker that uses the powerful Gemini 2.5 Pro model
    to synthesize all the information and provide a final, well-reasoned prediction.
    ...
    try:
        ...
        llm = ChatGoogleGenerativeAI(
            model="gemini-2.0-flash-thinking-exp",  # Most powerful model
            google_api_key=google_api_key,
            temperature=0.7  # Higher temperature for more creative synthesis
        )
        
        # Craft a comprehensive prompt for final aggregation
...
"""
        ...
        # Get the final analysis from the powerful LLM
        response = llm.invoke(prompt)
