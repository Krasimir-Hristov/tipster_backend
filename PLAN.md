...
- [x] **Стъпка 1.1: Инициализация на Python среда и зависимости**

  - [x] Създаване на виртуална среда (`.venv`).
  - [x] Активиране на виртуалната среда.
  - [x] Създаване на `requirements.txt` с необходимите пакети.
  - [x] Инсталиране на пакетите с `pip install -r requirements.txt`.

- [x] **Стъпка 1.2: Създаване на Django проект и приложение**

  - [x] Генериране на Django проект с име `tipster_project`.
  - [x] Генериране на Django приложение с име `api`.
  - [x] Добавяне на `rest_framework` и `api` приложението в `INSTALLED_APPS` в `settings.py`.

- [x] **Стъпка 1.3: Конфигуриране на API ендпойнт**
  - [x] Създаване на `urls.py` в `api` приложението.
  - [x] Дефиниране на основен URL ендпойнт `/api/analyze/` в `api/urls.py`.
  - [x] Включване на `api/urls.py` в главния `tipster_project/urls.py`.
  - [x] Създаване на празна View функция в `api/views.py`, която ще обработва заявките.
...
- [ ] **Стъпка 2.3: Създаване на специализираните "анализаторски" възли (Nodes)**
  ...
  - _Всяка от тези функции ще извиква евтин LLM (Gemini Flash) със силно специфичен prompt._
